    ...
    <table class="table table-bordered table-striped">
        ...
        <?php while ($usuario = mysqli_fetch_assoc($resultado)): ?>
            <tr>
                ...
                <td>
                    <!-- Exclusão: Botão desabilitado se for o utilizador logado para evitar auto-exclusão -->
                    <?php if ($usuario["id"] != $_SESSION["usuario_id"]): ?>
                    <a href="processa.php?acao=excluir_usuario&id=<?php echo $usuario[
                        "id"
                    ]; ?>"
                    class="btn btn-sm btn-danger"
                    onclick="return confirm('ATENÇÃO: Deseja realmente EXCLUIR o utilizador: <?php echo htmlspecialchars(
                        $usuario["usuario"],
                    ); ?>?');">
                    Excluir
                    </a>
